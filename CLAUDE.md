...
- `app/static/index.html` — dashboard UI (Chart.js, vanilla JS, auto-refreshes every 30s). Three-page SPA via hash routing: `#euphoria` (default — hero score, episode lines, 12 cards in 4 sections), `#disconnect` (disconnect score, visual 2x2 quadrant with positioned dot, 5 cards), and `#crypto` (BTC/ETH MA120 regime page — LONG/CASH badges, close-vs-MA120 dual-line charts, last signal date); page tabs under the header, cross-links in each hero
...
| Fundamentals | `track_fundamentals` | `erp_pct`, `multiple_expansion_pct`, `cape`, `cape_percentile`, `margins_pct`, `credit_gap_pct` | FRED (DGS10/CP/GDP/CMDEBT/BCNSDODNS), multpl.com CAPE scrape, yfinance SPY `.info`/`^GSPC`/`^TNX` |
| Crypto regime | `track_crypto` | `asset`, `close`, `ma_120`, `deviation_pct`, `vol_20_pct`, `regime`, `overlay_weight_pct` | yfinance BTC-USD/ETH-USD |
...
- `run_crypto` — live-only full-upsert (like margin debt): each run fetches ~560 days of BTC-USD/ETH-USD via yfinance (explicit start/end) and upserts ~440 daily rows per asset into `track_crypto` (PK date+asset) — self-healing, needs no per-date backfill, and is deliberately NOT in `BACKFILL_TABLES` (crypto trades 7 days/week; the NDX trading-day calendar doesn't apply). Computes MA120 regime (LONG/CASH), deviation %, 20d annualized vol, and the vol-overlay weight min(1, 50%/vol). Strategy source + evidence: `~/Desktop/crypto-ta-lab` (walk-forward validated 2026-07-07). Crypto is display-only — it does NOT feed the euphoria/disconnect composites.
- `run_fundamentals` — live-only (mixed monthly/quarterly sources with publication lags); each of its 5 components fails independently; 10Y yield falls back to yfinance `^TNX` when FRED `DGS10` times out (the container hits intermittent FRED read-timeouts — same issue that breaks `run_liquidity`). **Do NOT use Shiller's ie_data.xls — it stopped updating in 2023** (earnings end 2023-06); CAPE and S&P PE come from multpl.com monthly tables (current, back to 1871, regex scrape in `_multpl_series`). **Multiple expansion uses S&P price (`^GSPC`) ÷ corporate profits (FRED `CP`, trailing-4Q-avg to damp NIPA noise), 12-month change** — ~1-quarter lag, far fresher than multpl's ~9-month-lagged GAAP S&P PE (do NOT use the `s-p-500-pe-ratio` multpl series for this; it's stale). CAPE still comes from multpl's `shiller-pe` (current). **Credit-to-GDP gap uses Fed Z.1 (`CMDEBT` households + `BCNSDODNS` nonfinancial business, $ millions → billions) instead of BIS `CRDQUSAPABIS`** — same total-private-non-financial scope but Z.1 publishes ~a quarter sooner (~10wk after quarter-end vs BIS's longer lag). Scale differs (Z.1 2007 gap peaks ~+18 vs BIS ~+24) but both saturate past the `extreme=10` anchor and read negative today, so the swap doesn't move the current disconnect score. `run_fundamentals` also upserts native-cadence history (from 2010) for cape/cape_percentile (monthly) and multiple_expansion/margins/credit_gap (quarterly) into `track_metric_history`. The 5 disconnect cards chart that history as **stepped lines** (Chart.js `stepped:'before'`) over a fixed window (CAPE 72 months, quarterly metrics 28) — ERP stays a daily line. Margin debt is also stepped. This replaced flat single-value daily charts that looked broken.
